...
install_kernel_overlay() {
    ...
    sudo touch /etc/apt/sources.list.d/intel-asl.list
    echo -e "deb https://download.01.org/intel-linux-overlay/ubuntu jammy main non-free multimedia kernels\n\
    deb-src https://download.01.org/intel-linux-overlay/ubuntu jammy main non-free multimedia kernels" | sudo tee /etc/apt/sources.list.d/intel-asl.list
    ...
    sudo touch /etc/apt/preferences.d/intel-asl
    echo -e "Package: *\n\
