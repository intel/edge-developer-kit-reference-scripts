...
docker run -t -u root -d --rm --name notebooks --net=host -v /etc/group:/etc/group --device=/dev/dri --device=/dev/accel --group-add="$(stat -c "%g" /dev/dri/render* | head -n 1)" -v "${CURRENT_DIRECTORY}":/mnt -v /tmp/.X11-unix:/tmp/.X11-unix -e DISPLAY="$DISPLAY" -p 127.0.0.1:8888:8888 \
