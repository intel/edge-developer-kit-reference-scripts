...
install_openvino_docker(){
    ...
    if ! docker images | grep openvino/ubuntu24_dev; then
        ...
        docker tag openvino/ubuntu24_dev:latest openvino/ubuntu24_dev:devkit
    else
        echo "$S_VALID OpenVINO™ docker image already installed"
    fi

    echo -e "\n# Build OpenVINO™ with NPU docker image"
    if ! docker images | grep openvino_npu/ubuntu24_dev; then
        docker build --progress=plain --no-cache -t openvino_npu/ubuntu24_dev:latest -f Dockerfile .
        docker rmi openvino/ubuntu24_dev:devkit openvino/ubuntu24_dev:latest
    else
...
install_openvino_notebook_docker(){
    ...
    if ! docker images | grep openvino_notebook; then
        docker run -t -d --rm --name temp_notebooks -v "${CURRENT_DIRECTORY}":/mnt openvino_npu/ubuntu24_dev:latest
