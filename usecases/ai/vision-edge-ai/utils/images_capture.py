...
class VideoCapWrapper(ImagesCapture):
	def __init__(self, input, loop):
		self.loop = loop
		# Initialize with FFmpeg backend and VAAPI acceleration
		self.cap = cv2.VideoCapture(input, cv2.CAP_FFMPEG)
		if not self.cap.isOpened():
			raise InvalidInput(f"Can't open the video from {input}")

		# Enable FFmpeg HW acceleration (VAAPI)
		self.cap.set(cv2.CAP_PROP_HW_ACCELERATION, cv2.VIDEO_ACCELERATION_VAAPI)
		hw_accel_status = self.cap.get(cv2.CAP_PROP_HW_ACCELERATION)
		#print(f"FFmpeg HW Acceleration Requested: VAAPI")
		#print(f"FFmpeg HW Acceleration Status: {'Enabled' if hw_accel_status == cv2.VIDEO_ACCELERATION_VAAPI else 'Disabled'}")
