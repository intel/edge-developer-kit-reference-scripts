...
class Preprocesser:
    ...
    def crop(self, img_np_list, still=False, xsize=512, output_path='output_with_landmarks.jpg', threshold=(60, 50, 10, 50)):    # first frame for all video
        ...
        # Extract mouth landmarks (indices 48 to 67)
        mouth_landmarks = lm[48:68]
        mouth_x = [point[0] for point in mouth_landmarks]
        mouth_y = [point[1] for point in mouth_landmarks]
        
        # Calculate bounding box for the mouth with threshold
        left_threshold, right_threshold, top_threshold, bottom_threshold = threshold
        mouth_x_min = int(min(mouth_x)) - left_threshold
        mouth_x_max = int(max(mouth_x)) + right_threshold
        mouth_y_min = int(min(mouth_y)) - top_threshold
        mouth_y_max = int(max(mouth_y)) + bottom_threshold
        
        # Ensure the bounding box is within image boundaries
        mouth_x_min = max(mouth_x_min, 0)
        mouth_x_max = min(mouth_x_max, img_np.shape[1])
        mouth_y_min = max(mouth_y_min, 0)
        mouth_y_max = min(mouth_y_max, img_np.shape[0])
        ...
        return img_np_list, crop, quad, [mouth_x_min, mouth_x_max, mouth_y_min, mouth_y_max]
