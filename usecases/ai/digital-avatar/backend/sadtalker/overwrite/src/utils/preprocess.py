...
class CropAndExtract():
    ...
    def generate(self, input_path, save_dir, crop_or_resize='crop', source_image_flag=False, pic_size=256):
        ...
        if 'crop' in crop_or_resize.lower(): # default crop
            if not source_image_flag:
                ...
                for frame in x_full_frames:
                    _, _, _, mouth_coord = self.propress.crop([frame], still=True if 'ext' in crop_or_resize.lower() else False, xsize=512)
                    mouth_coords.append(mouth_coord)
                return None, None, None, mouth_coords
            else:
                x_full_frames, crop, quad, mouth_coords = self.propress.crop(x_full_frames, still=True if 'ext' in crop_or_resize.lower() else False, xsize=512)
        ...
        return coeff_path, png_path, crop_info, mouth_coords
