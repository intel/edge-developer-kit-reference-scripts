...
class OVWav2Lip:
    def __init__(self, avatar_path="assets/xy.png", device="GPU"):
    ...
    def inference(self, audio_path, reversed=False):
        ...
        gen = self.datagen(full_frames.copy(), mel_chunks, face_det_results)
        ...
        video_frames = len(self.full_frames)

        if not frames_generated == video_frames:
            leftover_frames_index = frames_generated % video_frames

            if frames_generated > video_frames:
                is_reversed = 0 if frames_generated // video_frames % 2 == 0 else 1
            else:
                is_reversed = 0
            if (is_reversed and not reversed) or (not is_reversed and reversed):
                leftover_frames = self.full_frames[:video_frames-leftover_frames_index][::-1].copy()
            else:
                leftover_frames = self.full_frames[leftover_frames_index:].copy()
        ...
        command = 'ffmpeg -y -i {} -i {} -strict -2 -q:v 1 {} > /dev/null 2>&1'.format(
