...
def cleanup(ssh, script_path):
    ...
    try:
        ...
        run_command(ssh, "docker stop automatic-speech-recognition && docker rm automatic-speech-recognition")
        run_command(ssh,"docker rmi -f automatic-speech-recognition")
...
def main():
    ...
    parser.add_argument('--usecase_script_path_stt', required=True, help='SUT script path.'
    ...
    base_path = os.path.normpath(args.usecase_script_path_stt).split(os.sep)[1]
    ...
    if sut_ssh:
        ...
        thm_script_folder = THM_SCRIPT_PATH + args.usecase_script_path_stt

        sut_script_folder = SUT_SCRIPT_PATH + args.usecase_script_path_stt
        ...
        while True:
            ...
            get_health_check_cmd = "curl http://localhost:5996/healthcheck"
            export_env_cmd = "export RENDER_GROUP_ID=$(getent group render | cut -d: -f3)"
            stdout, _ = run_command(sut_ssh, f"{export_env_cmd} && cd {sut_script_folder} && docker build -t automatic-speech-recognition . && {docker_run_cmd} && sleep 2m && {get_health_check_cmd}", ignore_stderr=True)
