...
def cleanup(ssh, script_path):
    ...
    try:
        ...
        run_command(ssh, "docker stop auto-ollama && docker rm auto-ollama")
        run_command(ssh,"docker rmi -f auto-ollama-app:latest")
...
def main():
    ...
    Command-line Arguments:
        ...
        --usecase_script_path_ollama (str): Usecase Script to run on SUT.
    """
    ...
    parser.add_argument('--usecase_script_path_ollama', required=True, help='SUT script path.'
    ...
    base_path = os.path.normpath(args.usecase_script_path_ollama).split(os.sep)[1]
    ...
    if sut_ssh:
        ...
        thm_script_folder = THM_SCRIPT_PATH + args.usecase_script_path_ollama

        sut_script_folder = SUT_SCRIPT_PATH + args.usecase_script_path_ollama
        ...
        docker_run_cmd ="docker run -d \
            ...
            -e OLLAMA_NUM_GPU=${OLLAMA_NUM_GPU:-999} \
        ...
        while True:
            ...
            get_health_check_cmd = "curl http://localhost:11434"
            stdout, _ = run_command(sut_ssh, f"cd {sut_script_folder} && docker build -t auto-ollama-app . && {docker_run_cmd} && sleep 1m && {get_health_check_cmd}", ignore_stderr=True)
            
            if stdout:
                last_stdout_line = stdout.split("\n")[-1]
                logger.info("Last line of stdout: %s", last_stdout_line)
                if PASS_MSG in last_stdout_line:
                    print("Ollama usecase successfully executed")
