...
:Author :
    Jessie Lim [jessie.sheau.ling.lim@intel.com]

:Description :
    Caller script to run automated devkit BKC installation test

:Last Modified by :
    Jessie Lim [jessie.sheau.ling.lim@intel.com]

:Last Modified Date : 14 Nov 2024
"""
...
def run_command(ssh, command, ignore_stderr=False):
    ...
    if REBOOT_MSG in last_stdout_line or SETUP_DONE_MSG in last_stdout_line:
        # Close the channels
...
def main():
    ...
    parser.add_argument('--script_path', required=True, help='SUT script path.'
    ...
    base_path = os.path.normpath(args.script_path).split(os.sep)[1]
    ...
    if sut_ssh:
        ...
        run_command(sut_ssh, f"mkdir -p {SUT_SCRIPT_PATH}{args.script_path}")
        ...
        while True:
            ...
            if stdout:
                ...
                if REBOOT_MSG in last_stdout_line:
                    logger.info("Rebooting SUT......")
                    run_command(sut_ssh, "sudo reboot")
                    time.sleep(60)  # Sleep for 60 seconds to allow the SUT to reboot

                    # Perform ping test to check if SUT is up
                    if ping_test(args.sut_ip):
                        logger.info("SUT is back online. Reconnecting via SSH......")
                        try:
                            sut_ssh = establish_ssh_connection(args.sut_ip, args.sut_username, args.sut_password)
                        except paramiko.SSHException as e:
                            logger.error("Failed to reconnect to SUT: %s", e)
                            cleanup(sut_ssh, f"/{base_path}")
                            sut_ssh.close()
                            sys.exit(1)
                        continue  # Reconnect and rerun the script
                    else:
                        logger.error("Ping test failed. SUT did not come back online after reboot")
                        cleanup(sut_ssh, f"/{base_path}")
                        sut_ssh.close()
                        sys.exit(1)
                elif SETUP_DONE_MSG in last_stdout_line:
                    logger.info("BKC Installation Test Pass.")
                    cleanup(sut_ssh, f"/{base_path}") # Perform post-test cleanup
                    sut_ssh.close() 
                    sys.exit(0)  # Exit the script with a success status code
            else:
                logger.error("BKC Installation Test Fail.")
                cleanup(sut_ssh, f"/{base_path}")
                sut_ssh.close()
                sys.exit(1)
    else:
